...
! *** Adaptive Quadrture
  ...
  PARAMETER (STACKSIZE = 10000,IL=1,IR=2,IE=3)
...
SUBROUTINE Stackops(stackpointer,stack,result,Func)
  ...
  INTEGER active_threads, work

  active_threads = 0
  work = 0

!$OMP parallel default(none) shared(result, stack, stackpointer, active_threads) private(abserror, l, r, m, est1, est2, eps, work)
10  DO WHILE  (stackpointer .GE. 1 .OR. active_threads .NE. 0)

! *** Pop next interval off stack.
!$OMP critical (pop)
     IF (stackpointer .EQ. 0) THEN
        work = 0
     ELSE
...
! *** ours
!$OMP critical (resultCalc)
        result = result + est2
        active_threads = active_threads - 1
!$OMP END critical (resultCalc)
! *** ours
     ELSE
